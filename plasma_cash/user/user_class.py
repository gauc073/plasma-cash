...
from plasma_cash.user.contract_instance import get_contract
from plasma_cash.user.exceptions import *
...
class User:
    ...
    def __init__(self, public_address, private_key, root_chain_provider=None, child_chain_provider=None, ):
        ...
        self.contract = get_contract(self.web3)
    ...
    def add_address(self, public_address, private_key):
        try:
            ...
            if self.address is not None:
                ...
                pass
        except InvalidKeyAddressException:
            raise InvalidKeyAddressException('invalid Key or Address Exception')
    ...
    def get_uid(self, tx_hash):
        ...
        myfilter = self.contract.eventFilter('Deposit',
    ...
    def validate_address(public_address, private_key):
        if str(private_key).startswith("0x"):
            private_key = str(private_key).split("0x")[1]
        private_key_bytes = bytes.fromhex(str(private_key))
        public_key_hex = keys.PrivateKey(private_key_bytes).public_key
        public_key_bytes = bytes.fromhex(str(public_key_hex)[2:])
        return str(public_address).lower() == str(keys.PublicKey(public_key_bytes).to_address()).lower()
